...
then
    ...
    echo "${RED}Run 'sh test.sh' or 'firebase emulators:exec \"sh run_tests.sh\"' instead$DEF".
    exit 1;
fi
...
echo -e 🏗 Setting up
node test/firestore_test/lib/setup_tests.js $password $adminEmailAddress $emailVerifiedEmail $secondEmail
